...
describe StuffToDo, '#available for user' do
  ...
  before(:each) do
    @user = mock_model(User)
    @find_options = { :conditions => ['1=1 AND (issue_statuses.is_closed = ?::boolean) AND (projects.status = ?) AND (assigned_to_id = ?)',false, 1, @user.id ], :include => [:status, :priority, :project], :order => 'issues.created_on DESC'}
  end
...
describe StuffToDo, '#available for status' do
  ...
  before(:each) do
    ...
    @find_options = { :conditions => ['1=1 AND (issue_statuses.is_closed = ?::boolean) AND (projects.status = ?) AND (issue_statuses.id = (?))', false, 1, @status.id ], :include => [:status, :priority, :project], :order => 'issues.created_on DESC'}
  end
...
describe StuffToDo, '#available for priority' do
  ...
  before(:each) do
    ...
    @find_options = { :conditions => ['1=1 AND (issue_statuses.is_closed = ?::boolean) AND (projects.status = ?) AND (enumerations.id = (?))', false, 1, @priority.id ], :include => [:status, :priority, :project], :order => 'issues.created_on DESC'}
  end
